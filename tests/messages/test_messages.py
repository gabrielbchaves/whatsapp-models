...
class TestTextMessage:
    def test_basic(self) -> None:
        ...
        msg = TextMessage.model_validate({**BASE, "text": {"body": "Olá!"}})
        assert msg.type == MessageType.text
        assert msg.to == RECIPIENT
        assert msg.text.body == "Olá!"

    def test_preview_url_default_false(self) -> None:
        """TextMessage.text.preview_url defaults to False."""
        msg = TextMessage.model_validate({**BASE, "text": {"body": "https://example.com"}})
    ...
    def test_missing_body_raises(self) -> None:
        ...
        with pytest.raises(ValidationError):
            TextMessage.model_validate({**BASE, "text": {}})

    def test_messaging_product_fixed(self) -> None:
        """TextMessage.messaging_product is always 'whatsapp'."""
        msg = TextMessage.model_validate({**BASE, "text": {"body": "hi"}})
        assert msg.messaging_product == MessagingProduct.whatsapp

    def test_recipient_type_default(self) -> None:
...
class TestAudioMessage:
    def test_with_id(self) -> None:
        ...
        msg = AudioMessage.model_validate({**BASE, "audio": {"id": "abc123"}})
    ...
    def test_with_link(self) -> None:
        ...
        msg = AudioMessage.model_validate({**BASE, "audio": {"link": "https://example.com/audio.mp3"}})
...
class TestLocationMessage:
    ...
    def test_requires_name_and_address(self) -> None:
        ...
        with pytest.raises(ValidationError):
            LocationMessage.model_validate({**BASE, "location": {"latitude": 0.0, "longitude": 0.0}})

    def test_requires_lat_lng(self) -> None:
        """LocationMessage raises ValidationError when coordinates are missing."""
        with pytest.raises(ValidationError):
            LocationMessage.model_validate({**BASE, "location": {}})


class TestReactionMessage:
    def test_basic(self) -> None:
        """ReactionMessage stores message_id and emoji correctly."""
        msg = ReactionMessage.model_validate({**BASE, "reaction": {"message_id": "wamid.abc", "emoji": "👍"}})
    ...
    def test_requires_message_id_and_emoji(self) -> None:
        ...
        with pytest.raises(ValidationError):
            ReactionMessage.model_validate({**BASE, "reaction": {"message_id": "wamid.abc"}})
...
class TestTemplateMessage:
    def test_basic(self) -> None:
        ...
        msg = TemplateMessage.model_validate(
            {**BASE, "template": {"name": "hello_world", "language": {"code": "pt_BR"}}},
        )
        assert msg.type == MessageType.template
        assert msg.template.name == "hello_world"
        assert msg.template.language.code == "pt_BR"

    def test_requires_name_and_language(self) -> None:
        """TemplateMessage raises ValidationError when language is missing."""
        with pytest.raises(ValidationError):
            TemplateMessage.model_validate({**BASE, "template": {"name": "hello_world"}})

    def test_text_parameter_with_parameter_name(self) -> None:
    ...
    def test_text_parameter_name_optional(self) -> None:
...
class TestInteractiveReplyButtonsMessage:
    def test_basic(self) -> None:
        ...
        msg = InteractiveMessage.model_validate(
            {
                **BASE,
                "interactive": {
                    "type": "button",
    ...
    def test_max_three_buttons(self) -> None:
        ...
        with pytest.raises(ValidationError):
            InteractiveMessage.model_validate(
                {
                    **BASE,
                    "interactive": {
                        "type": "button",
                        ...
                        "action": {
                            "buttons": [
                                {"type": "reply", "reply": {"id": f"btn{i}", "title": f"Op {i}"}} for i in range(4)
                            ]
                        },
                    },
                }
...
class TestOutgoingMessageUnion:
    adapter: TypeAdapter[OutgoingMessage] = TypeAdapter(OutgoingMessage)
    ...
    def test_resolves_interactive(self) -> None:
...
class TestMessageStatusUpdate:
    ...
    def test_with_typing_indicator(self) -> None:
    ...
    def test_typing_indicator_type_fixed(self) -> None:
